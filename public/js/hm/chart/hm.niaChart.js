...
NiaChart.prototype = function() {
    ...
    var setSeries = function (_series) {
        ...
        for(var i = 0; i < _series.length; i++) {
            this.chart.addSeries({name: _series[i].name, type:'column'}, false) //스텍차트 type:'column'
        }
        this.chart.redraw();
    };
    ...
    var absHtmlTooltipFormatter = function() {
        ...
        $.each(this.points, function() {
            s += '<tr><td style="color: ' + this.series.color + '">' + this.series.name + ':</td>' +
                '<td style="text-align: right">' + HmUtil.commaNum(Math.abs(this.y.toFixed(2))) + suffix + '</td></tr>';
            total = this.total;
        });
