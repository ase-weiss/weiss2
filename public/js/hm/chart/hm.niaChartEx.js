 ...
 * @constructor
 */
var NiaChartEx = function(chartId) {
...
NiaChartEx.prototype = function() {
    ...
    var setSeries = function (_series) {
        ...
        for(var i = 0; i < _series.length; i++) {
            this.chart.addSeries({name: _series[i].name, type:'column'}, false)
        }
        this.chart.redraw();
    };
    ...
    var absHtmlTooltipFormatter = function() {
        ...
        $.each(this.points, function() {
            s += '<tr><td style="color: ' + this.series.color + '">' + this.series.name + ':</td>' +
                '<td style="text-align: right">' + HmUtil.convertUnit1024(Math.abs(this.y.toFixed(2))) + suffix + '</td></tr>';
            total = this.total;
        });
        s += '<tr><td style="color: #9579da">Total: </td>' +
            '<td style="text-align: right">' + HmUtil.convertUnit1024(Math.abs(total.toFixed(2))) + suffix + '</td></tr>';
        s += '</table>';
        return s;
    };
