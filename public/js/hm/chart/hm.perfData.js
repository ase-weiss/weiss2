...
PerfData.prototype = function() {
    ...
    //응답시간 테이블 변경
    var searchDevResptimePerf = function(_this, params, searchDataResult){
        Server.post('/main/popup/rawPerfChart/getPerfChartForResptime.do', {
            data: params,
            success: function(result) {
                searchDataResult.call(_this, params, result);
            }
    ...
    var searchEsDevPerf = function(_this, params, searchDataResult){
        ...
        param.body = {
            ...
            from: 0, size: 10000, sort: [{'YMDHMS': 'asc'}], aggs: {'group_by_ymdhms': {'terms': {'field':'YMDHMS'} , aggs: {'group_by_mngno': {'terms': {'field':'MNG_NO'} }}}}
        };
        if(param.itemType != 6) {
            ...
            if (!resp.hasOwnProperty('hits')) {
                ...
                if ((resp.hits.total > allResult.length)) {
                    ...
                    $.each(allResult, function(i,v){
                        data.push({ DT_YMDHMS: HmHighchart.change_date(v.YMDHMS_FT).getTime(), RATE: parseInt(v[param.yValue])});
                    });
    ...
    return {
        ...
        searchDevResptimePerf: searchDevResptimePerf  //응답시간
    }
