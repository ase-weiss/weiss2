...
//			                	this.showLoading();  // show loading message
    ...
    addAxisPlotLine: function(axis, id, limitValue, labelText) {
        axis.addPlotLine({ id: id, value: limitValue,
        ...
        if(dataRows != null && dataRows.length) {
            $.each(dataRows, function(idx, item) {
                if(idx == 0) { // grid.columns
                    for(var x in item) {
                        if(x == 0) {
                            params.cols.push({text: item[x], datafield: 'val' + x, cellsalign: 'center'});
                        }
                        else {
                            params.cols.push({text: item[x], datafield: 'val' + x, cellsalign: 'right', cellsrenderer: unitrenderer});
                        }
                    }
