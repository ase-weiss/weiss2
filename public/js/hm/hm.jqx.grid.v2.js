...
//				filterable: true,  				/* loading slow */
    ...
    var create = function(options, ctxmenuType, ctxmenuIdx) {
        ...
        console.log("source", options.source, "options", options.columns);
    ...
    var generateColumns = function(editable) {
        ...
        $.each(datafields, function(i, v) {
            if(v.hasOwnProperty('text')) {
                ...
                // grid.editable==true이고 column.editable==true인 경우 header column의 폰트컬러를 blue로 표시한다. -> 대표님이 Grid 타이틀 gray로 변경하라고 하심. 2020년도
                if(newCol.editable) {
